...
Issues and specs for this repo live as GitHub issues in `jacobmalone28/snack-gpt`. Use the `gh` CLI for all operations.
...
- **Blocking**: use GitHub's native issue dependencies. Add an edge with `gh api --method POST repos/<owner>/<repo>/issues/<child>/dependencies/blocked_by -F issue_id=<blocker-db-id>`, where `<blocker-db-id>` is the blocker's numeric database ID from `gh api repos/<owner>/<repo>/issues/<number> --jq .id`. Where dependencies are unavailable, use a `Blocked by: #<number>` line at the top of the child body.
